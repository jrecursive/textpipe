...
import com.alienobject.textpipe.services.util.StringUtils;
...
public class TextUtils {
    private static String[] months_short = {"jan", "feb", "mar", "apr", "may", "jun", "jul",
            "aug", "sep", "oct", "nov", "dec"};
    private static String[] months_long = {"january", "february", "march", "april", "may",
    ...
    public static String sha1(String s)
        ...
        return StringUtils.bytes2hex(md.digest());
    }
    ...
    /*
    private static JSONObject loadjsonfile(String fn) throws Exception {
        return new JSONObject(getfile(fn));
    }
    */

    private static JSONObject loadjsonurl(String u) throws Exception {
        return new JSONObject(geturl(u));
    }

    private static void dbg(String s) {
        if (trace) System.out.println(s);
    }
    //// date mining / finding / parsing / test code

    public ArrayList<Long> findDatesInURL(String u) {
        ArrayList<Long> ar = new ArrayList<Long>();
        return ar;
    }

    public ArrayList<Long> findDatesInHTML(String content) {
        ArrayList<Long> ar = new ArrayList<Long>();
        return ar;
    }
    ...
    ArrayList<Long> findDatesInText(String u) throws Exception {
        ArrayList<Long> ar = new ArrayList<Long>();
        int i, j, k;
        ...
        */
        ...
        for (String u_chunk : u_chunks) {
            ...
            if (!is_num) {
                ...
                if (ds.length() == 3) {
                    ...
                    if (found_year) {
                        dbg("\t\t wow! i found one (year, month) via sm1 (transition to !is_num)!" +
                                " (" + pos_year + ", " +
                                pos_month + ", " + pos_day + ")");
                ...
                if (u_chunk.length() == 12) {
                    ...
                    if (Integer.parseInt(u_chunk.substring(4, 6)) < 70) {
                        s6_yyyy = "20" + u_chunk; // guess 20XX (we're good until 2069, ;p)
                    } else {
                        s6_yyyy = "19" + u_chunk;
                    }
                    ...
                    if (v1 > 1900 &&
                        ...
                        if (!found_month &&
                            ...
                            if (found_month) {
                                String s4_2then4 = u_chunk + "20" + (pos_month < 10 ? "0" : "") + pos_month;
                    ...
                    if (found_month &&
                        ...
                        if (pos_v < 70) {
                            if ((2000 + pos_v) <= cal_year) {
                                ...
                                dbg("\t\t\tfailed common sense test (@2069 test #2), " + (2000 + pos_v) + " > " + cal_year + ", ignoring");
                            }
                        } else {
                            pos_year = 1900 + pos_v;
                        }
                        if (pos_year <= cal_year) {
                            found_year = true;
                            dbg("found_year by implication (already have month, day) via shorthand 2 digit year token");
                        } else {
                            pos_year = 0;
                            found_year = false;
                            dbg("unless we've found an e-time-machine, " + pos_year + " isn't a possible date, ignoring");
                        }
                        ...
                        if (pos_v > 0 &&
                            ...
                            if (found_year) {
                                ...
                                dbg("\t\tdon't have a year either yet, so looks like we're SOL.");
                            }
                        }
                    }
                } else {
                ...
                if (tval > 0) {
                    ...
                    tval = 0;
                        ...
                        found_year &&
                                found_month &&
                                found_day) {
                    dbg("\t\t wow! i found one via sm1! (" + pos_year + ", " +
                            pos_month + ", " + pos_day + ")");
                    rcal = Calendar.getInstance();
                    rcal.set(pos_year, pos_month - 1, pos_day);
                    dbg("\t\t**rcal = " + rcal.toString());
            ...
            // last chance date dance
            ch_c++;
            dbg("ch_c = " + ch_c + ", ch_sz = " + ch_sz);
            if (found_year &&
                    found_month &&
                    ch_c == ch_sz - 1) {
                dbg("\t\t wow! i found one (year, month) via sm1 (last u_chunk try)! (" + pos_year + ", " +
                        pos_month + ", " + pos_day + ")");
                rcal = Calendar.getInstance();
                if (!found_day) {
                    pos_day = 1;
        ...
        if (found_year) {
            dbg("\t\t wow! i found something via sm1 (last chance)! (" + pos_year + ", " +
                    pos_month + ", " + pos_day + ")");
            rcal = Calendar.getInstance();
            if (!found_day) {
                pos_day = 1;
        ...
        for (long time_v : ar) {
            ...
            if (rcal.get(Calendar.YEAR) < 1970 ||
                    rcal.get(Calendar.YEAR) > cal_year)
                continue;
            ar2.add(time_v);
        }
        return ar2;
    ...
    long brute_force_date(String s, int s_valid) {
        Calendar cal = Calendar.getInstance();
        int cal_day = cal.get(Calendar.DATE);
        int cal_month = cal.get(Calendar.MONTH);
        int cal_year = cal.get(Calendar.YEAR);
    ...
    private ArrayList<Long> coerceDatesFromText(String u) throws Exception {
        ...
        ArrayList<Long> ar2 = findDatesInText(u2);
    ...
    private ArrayList<Long> coerceDatesFromURL(String url) throws Exception {
        ...
        ParserCallback parserCallback = new ParserCallback() {
            ...
            public void handleStartTag(Tag tag, MutableAttributeSet attribute, int pos) {
                for (Enumeration e = attribute.getAttributeNames(); e.hasMoreElements();) {
                    ...
                    if (("" + attr_v).toLowerCase().indexOf("date") != -1 ||
                        ...
                        dbg("handleStartTag <" + pos + ">: tag = " + tag + ", attr_nm = " + attr_nm + " -> " + attr_v);
                        parsingDates = true;
                    }
                }
            }
            ...
            public void handleSimpleTag(Tag tag, MutableAttributeSet a, final int pos) {
                //dbg("handleSimpleTag <" + pos + ">: tag = " + tag);
            }

            public void handleComment(final char[] data, final int pos) {
                //dbg("handleComment <" + pos + ">: data = " + new String(data));
            }

            public void handleError(final java.lang.String errMsg, final int pos) {
                //dbg("handleError <" + pos + ">: " + errMsg);
            }
        };
    ...
    long coerceDates(String url, String content) throws Exception {
        ArrayList<Long> dates_url = coerceDatesFromText(url);
        dbg("scanning content for url: " + url);
        dbg("-- content dates --");
        ArrayList<Long> dates_content = coerceDatesFromURL(content);
        ...
        if (ld_ar.size() > 0) {
            ...
            for (long likely_date : ld_ar) {
                ...
                dbg("WTF likely_date = " + likely_date);
            }
            Calendar lcal = Calendar.getInstance();
            lcal.setTimeInMillis(newest_tval);
            dbg("[" + (ld_ar.size() == 1 ? "only" : "newest") + "] most likely overall date: " +
                    (lcal.get(Calendar.MONTH) + 1) + "/" +
                    lcal.get(Calendar.DAY_OF_MONTH) + "/" +
                    lcal.get(Calendar.YEAR) + "    " + url + "    out of " + ld_ar.size() + " possible values");
    ...
    public static void main(String args[]) throws Exception {
        ...
        String url = "http://news.prnewswire.com/ViewContent.aspx?ACCT=109&STORY=/www/story/04-08-2009/0005002844&EDATE=";
        long final_dt = tu.coerceDates(url, url);
        ...
        String solr_url = "http://localhost:9999/solr/select?indent=on&version=2.2&q=url%3A[*+TO+*]%0D%0A-possible_date%3A[*+TO+*]%0D%0A&start=9000&rows=" + numUrls + "&fl=url&qt=standard&wt=json&explainOther=&hl.fl=";
        
        JSONObject j_solr = loadjsonurl(solr_url);
        JSONArray j_solr_docs = j_solr.getJSONObject("response").getJSONArray("docs");
        ArrayList<String> urls = new ArrayList<String>();
        ArrayList<String> urls_hash = new ArrayList<String>();
        for(int i=0; i<j_solr_docs.length(); i++) {
            JSONObject j_url = j_solr_docs.getJSONObject(i);
            String url = j_url.getString("url");
            if (urls_hash.contains(sha1(url))) continue;
            urls.add(url);
            urls_hash.add(sha1(url));

            //System.out.println("-- url dates <" + url + "> --");
            //ArrayList<Long> dates_url = tu.coerceDatesFromText(url);
            //dbg("scanning content for url: " + url);
            //System.out.println("-- content dates --");
            //ArrayList<Long> dates_content = tu.coerceDatesFromURL(url);
