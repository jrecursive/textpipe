package com.alienobject.textpipe.spiders;

import com.alienobject.textpipe.nlp.HTMLTextExtractor;
import com.alienobject.textpipe.services.http.HTTPRequest;
import com.alienobject.textpipe.services.http.HTTPResponse;
import com.alienobject.textpipe.services.http.HTTPService;
...
public class URLTools {
    ...
    public HashMap dumpHTMLMetadata(String s)
        ...
        byte[] byteArray = s.getBytes("UTF-8"); // "ISO-8859-1"); // choose a charset
    ...
    public static List<String> getLinksFromURL(String u) throws Exception {
        HTTPRequest fetchRequest = new HTTPRequest(u);
        HTTPResponse fetchResponse = HTTPService.getInstance().request(fetchRequest);
        return getLinksFromHTML(fetchResponse.getBody());
    }

    public static List<String> getLinksFromHTML(String content) throws Exception {
        StringReader reader = new StringReader(content);
        List<String> links = URLTools.extractLinks(reader);
        /*
        for (String link : links) {
            System.out.println("URLTools.getLinksFromHTML: " + link);
        }
        */
        return links;
    }
    ...
    public static HashMap<String, String> getTextFromURL(String u) throws Exception {
        HTTPRequest fetchRequest = new HTTPRequest(u);
        HTTPResponse fetchResponse = HTTPService.getInstance().request(fetchRequest);
        return getTextLinksFromHTML(fetchResponse.getBody());
    }

    public static void getdottree(String u) throws Exception {
        HTTPRequest fetchRequest = new HTTPRequest(u);
        HTTPResponse fetchResponse = HTTPService.getInstance().request(fetchRequest);
        String content = fetchResponse.getBody();
        ...
        StringReader reader;
        byte[] byteArray = c2.getBytes("UTF-8"); // todo: choose a charset
        ...
        URLTools url_toolz = new URLTools();
    ...
    public static HashMap<String, String> getTextLinksFromHTML(String content) throws Exception {
        StringReader reader = new StringReader(content);
        HashMap<String, String> links = URLTools.extractTextAndLinks(reader);
